...
def pformat(
    object, indent=1, width=80, depth=None, enable_pickle=ENABLE_PICKLE_DEFAULT
):
    """Format a Python object into a pretty-printed representation."""
    printer = PrettyPrinter(
        indent=indent, width=width, depth=depth, enable_pickle=enable_pickle
    )
...
class PrettyPrinter:
    ...
    def _format(
        self, object, stream, indent, allowance, context, level, enable_pickle=None
    ):
        if enable_pickle is None:
            enable_pickle = self.enable_pickle
        level = level + 1
        ...
        if enable_pickle and _pickleable(typ, object):
            ...
            context[objid] = 1
            indent = indent + self._indent_per_level
            self._format(args[0], stream, indent, allowance + 1, context, level)
            if length > 1:
                for ent in args[1:]:
                    if sepLines:
                        write(",\n" + " " * indent)
    ...
    def format(self, object, context, maxlevels, level, enable_pickle=None):
        ...
        """
        return _safe_repr(
            object, context, maxlevels, level, enable_pickle=enable_pickle
        )
